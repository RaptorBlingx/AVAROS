...
class ProfileMixin:
    ...
    def _scoped_key(self, prefix: str, item: str) -> str:
        ...
        if profile == self.BUILTIN_MOCK_PROFILE:
            raise ValidationError(
                message=(
                    "Cannot write settings for "
                    "built-in mock profile"
                ),
                field="profile",
                value=profile,
            )
    ...
    def _migrate_global_settings_to_profile(self) -> None:
        """Migrate global settings keys to active profile scope.

        Runs once on ``initialize()``. Idempotent — if scoped
        keys exist or no global keys found, skips silently.
        """
        active = self.get_active_profile_name()
        ...
        migrated = 0
        for old_pfx, new_pfx in prefixes:
            migrated += self._migrate_prefix(old_pfx, new_pfx)
    ...
    def _migrate_prefix(
        ...
        """Move unscoped keys to a profile-scoped namespace.

        Args:
            old_prefix: Global key prefix to scan.
            new_prefix: Scoped key prefix to write.
